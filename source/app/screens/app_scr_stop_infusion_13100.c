...
#define STOP_INFUSION_ID_W			160
#define STOP_INFUSION_ID_H          24
...
static const GUI_WIDGET_CREATE_INFO _aDialogCreate[] = {
  { WINDOW_CreateIndirect, "app_scr_stop_infusion_13100", ID_WINDOW_0, 0, 0, 256, 64, 0, 0x0, 0 },
};
...
static void put_select_stop_infusion(bool is_stop)
{
	...
	if(is_stop == 1)
	{
		hWin = WM_GetDialogItem(ui_get_current_hwin(), STOP_INFUSION_ID_IMG);	
		IMAGE_SetBitmap(hWin, get_image(kImgSelect1));

		hWin = WM_GetDialogItem(ui_get_current_hwin(), STOP_INFUSION_ID_STR_YES);	
		TEXT_SetBkColor(hWin, GUI_WHITE);
		TEXT_SetTextColor(hWin, GUI_BLACK);
		WM_BringToTop(hWin);		
		
		hWin = WM_GetDialogItem(ui_get_current_hwin(), STOP_INFUSION_ID_STR_NO);	
	...
	{
		hWin = WM_GetDialogItem(ui_get_current_hwin(), STOP_INFUSION_ID_IMG);	
		IMAGE_SetBitmap(hWin, get_image(kImgSelect2));
...
static void _cbDialog(WM_MESSAGE * pMsg)
{
	...
	switch (pMsg->MsgId) 
	{
		case WM_INIT_DIALOG:
		    ...
		    x = (DISPLAY_WIDTH - STOP_INFUSION_ID_W)/2;
			y = 20;

			//alarm image
			hWin = IMAGE_CreateEx(x, y, STOP_INFUSION_ID_W, STOP_INFUSION_ID_H, pMsg->hWin, WM_CF_SHOW,  
			                      IMAGE_CF_AUTOSIZE, STOP_INFUSION_ID_IMG);
			IMAGE_SetBitmap(hWin, get_image(kImgSelect1));

			GUI_SetFont(get_font(14));
			ptr = get_string(kStrYes);
			strlen = GUI_GetStringDistX(ptr);
			//yes
			hWin = TEXT_CreateEx(x + (STOP_INFUSION_ID_W/2 - strlen)/2, y+5, strlen, 14, pMsg->hWin, 
				                 WM_CF_SHOW, GUI_TA_VCENTER|GUI_TA_HCENTER,  STOP_INFUSION_ID_STR_YES, get_string(kStrYes));
			...
			//No
			hWin = TEXT_CreateEx(x + STOP_INFUSION_ID_W/2+(STOP_INFUSION_ID_W/2 - strlen)/2, y+5, strlen, 14, pMsg->hWin,  
				                 WM_CF_SHOW, GUI_TA_VCENTER|GUI_TA_HCENTER,  STOP_INFUSION_ID_STR_NO, get_string(kStrNo));
			TEXT_SetFont(hWin, get_font(14));
			TEXT_SetBkColor(hWin, GUI_WHITE);
			TEXT_SetTextColor(hWin, GUI_BLACK);
			WM_BringToTop(hWin);
...
//			put_select_stop_infusion(msg->CursorIndex);
//			break;
//
