...
#define ID_PLUS_OFFSET_1		(GUI_ID_USER + 0x01)
#define ID_PLUS_OFFSET_2		(GUI_ID_USER + 0x02)
#define ID_PLUS_OFFSET_3		(GUI_ID_USER + 0x03)
#define ID_PLUS_OFFSET_4		(GUI_ID_USER + 0x04)
#define ID_PLUS_OFFSET_5		(GUI_ID_USER + 0x05)

#define ID_PLUS_IMG_SELECT	(GUI_ID_USER + 0x06)
...
static const GUI_WIDGET_CREATE_INFO _aDialogCreate[] = {
  { WINDOW_CreateIndirect, "app_scr_test_plus_offset", ID_WINDOW_0, 0, 0, 256, 64, 0, 0x0, 0 },
};
...
static void move_plus_offset_cusor(int cursor)
{
	WM_HWIN hWin;
	if((cursor < 1) || (cursor > 5)) return;
	hWin = WM_GetDialogItem(ui_get_current_hwin(), ID_PLUS_IMG_SELECT);
	WM_MoveChildTo(hWin, get_image_x(cursor), 52);
}

static void change_plus_offset_data(int cursor, int data)
{
	WM_HWIN hWin;
	char buff[4];
	if((cursor < 1) || (cursor > 5)) return;
	...
	switch (pMsg->MsgId) 
	{
		case WM_INIT_DIALOG:
			...
			hWin = IMAGE_CreateEx(get_image_x(0), 52, bitmap->XSize, bitmap->YSize, pMsg->hWin, 
				                  WM_CF_SHOW,  IMAGE_CF_AUTOSIZE, ID_PLUS_IMG_SELECT);
		...
		case WM_PAINT:
			...
			GUI_DispStringInRect("OFFSET", &rect, GUI_TA_HCENTER|GUI_TA_VCENTER);
			//.
			GUI_SetFont(get_font(40));
			charlen = GUI_GetStringDistX("0");
			...
			//
			ui_rect_init_by_size(rect, get_item_x(5) - charlen - 5, 16, GUI_GetStringDistX("."), 40);
			GUI_DispStringInRect("-", &rect, GUI_TA_LEFT|GUI_TA_VCENTER);
...
//			move_plus_offset_cusor(msg->CursorIndex);
//			break;
//
//		case SCR_VALUE_ALTER:
//			move_plus_offset_cusor(msg->CursorIndex);
//			change_plus_offset_data(msg->CursorIndex, msg->DItem[0]);
//			break;
//		default:
//			break;
//	}
