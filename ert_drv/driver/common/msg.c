...
static SszSafeQueue g_msg_queue_inter;
static SszSafeQueue *g_msg_queue = &g_msg_queue_inter;
static MsgHandler g_msg_handler_arr[kMsgIDMax];
static MsgHandler g_notify_msg_handler_arr[kNotifyMsgIDMax];
...
void msg_init() {
	ssz_safe_queue_init(g_msg_queue, g_msg_nodes, ssz_array_size(g_msg_nodes),
	                    sizeof(g_msg_nodes[0]));
	event_set_handler(kEventReceiveNewMsg, msg_handle_all);
	ssz_assert(kMsgIDMax <= 256);
	ssz_assert(kNotifyMsgIDMax <= 256);
}
// get the first msg, if no msg, will return NULL
MsgOne *msg_font() {
	return (MsgOne *)ssz_safe_queue_front(g_msg_queue);
}

// delete first msg
void msg_pop() {
	ssz_safe_queue_pop_front(g_msg_queue);
}
...
// handle notify msg
void msg_notify_handle(MsgOne *one_msg) {
	ssz_assert(one_msg->msg_id < kNotifyMsgIDMax);
	if (g_notify_msg_handler_arr[one_msg->msg_id]) {
		g_notify_msg_handler_arr[one_msg->msg_id](one_msg->param);
	}
}
...
void msg_handle_all(void) {
	...
	while (!ssz_safe_queue_is_empty(g_msg_queue)) {
		one_msg = (MsgOne *)ssz_safe_queue_front(g_msg_queue);
		msg_handle(one_msg);
		ssz_safe_queue_pop_front(g_msg_queue);
	}
}
...
static void msg_post(MsgID msg, MsgParam param) {
	...
	one_msg.msg_id = msg;
	one_msg.param = param;
	if (ssz_safe_queue_push_back(g_msg_queue, &one_msg)) {
...
void msg_post_no_param(MsgID msg) {
	MsgParam msg_param;
	msg_param.int_param = 0;
...
void msg_notify_no_param_at_once(NotifyMsgID msg) {
	MsgOne one_msg;
	one_msg.msg_id = msg;
	one_msg.param.int_param = 0;
	msg_notify_handle(&one_msg);
}
void msg_notify_int_at_once(NotifyMsgID msg, int32_t param) {
	MsgOne one_msg;
	one_msg.msg_id = msg;
	one_msg.param.int_param = param;
	msg_notify_handle(&one_msg);
}
void msg_notify_addr_at_once(NotifyMsgID msg, const void *param) {
	MsgOne one_msg;
	one_msg.msg_id = msg;
	one_msg.param.addr_param = param;
	msg_notify_handle(&one_msg);
}
void msg_notify_two_param_at_once(NotifyMsgID msg, int16_t param1,
                                  int16_t param2) {
	MsgOne one_msg;
	one_msg.msg_id = msg;
...
// set handler function for notify msg
void msg_set_notify_handler(NotifyMsgID msg, MsgHandler func) {
	ssz_assert(msg < kNotifyMsgIDMax);
	g_notify_msg_handler_arr[msg] = func;
}
...
void msg_clear_all(void) {
	ssz_safe_queue_clear(g_msg_queue);
}

// is not exist msg
bool msg_is_empty(void) {
	return ssz_safe_queue_is_empty(g_msg_queue);
}

// return:  <0 when data1<data2, 0 when same, >0 when data1>data2
int msg_compare_by_msg_id(const void *data1, const void *data2,
                          size_t data_size) {
...
bool msg_is_exist(MsgID msg) {
	MsgOne one_msg;
	one_msg.msg_id = msg;
	if (ssz_safe_queue_find(g_msg_queue, &one_msg, msg_compare_by_msg_id) !=
	    -1) {
		return true;
	} else {
		return false;
