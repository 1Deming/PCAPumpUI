...
#endif
static bool g_tmp_buff_is_used[kSszTempAllocBuffTypeMax] = {0};
/************************************************
* Function 
************************************************/
...
{
	switch (alloc_type) {
	case kSszTempAllocBackupBuffWhenWriteFlash:
		ssz_check(ssz_array_size(g_backup_tmp_buff_when_write_flash) >= alloc_size);
		break;
	case kSszTempAllocCommonBuff:
...
void ssz_tmp_buff_free(void* alloc_buff) {
	TempAllocBuffType alloc_type = kSszTempAllocCommonBuff;

	if (alloc_buff == g_backup_tmp_buff_when_write_flash) {
		alloc_type = kSszTempAllocBackupBuffWhenWriteFlash;
	}
	else if (alloc_buff == g_common_tmp_buff) {
		alloc_type = kSszTempAllocCommonBuff;
	}else{
		ssz_check(0);
	}
