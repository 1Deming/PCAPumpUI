...
// get the data pointer, return NULL if not support
const uint8_t* ssz_fget_data_pointer(SszFile* file) {
	uint8_t* ret = NULL;
	if (ssz_is_result_ok(
	        ssz_ioctl(file, SSZ_IOCTL_GET_INTERNAL_DATA_POINTER, &ret))) {
		return ret;
	} else {
		return NULL;
	}
...
// return: kSszSuccess->success  other->have error, can Refer ErrorCode
SszResult ssz_fskip_read(SszFile* file, int buff_size) {
	return ssz_fskip_read_ex(file, buff_size, NULL);
}
SszResult ssz_fskip_read_ex(SszFile* file, int buff_size,
                            int* real_skip_buff_size) {
	if (ssz_is_result_ok(ssz_ioctl(file, SSZ_IOCTL_SKIP_READ, &buff_size))) {
		if (real_skip_buff_size) {
			*real_skip_buff_size = buff_size;
		}
		return kSszSuccess;
	} else {
		int real_skip_buff_size2 = 0;
		SszResult ret = kSszSuccess;
		uint8_t sz;
		for (int i = 0; i < buff_size; i++) {
			ret = ssz_fread(file, &sz, 1);
			if (ret != kSszSuccess) {
				break;
			}
			real_skip_buff_size2++;
		}
		if (real_skip_buff_size) {
			*real_skip_buff_size = real_skip_buff_size2;
		}
		return ret;
	}
}
