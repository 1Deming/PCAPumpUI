...
#define INFUSION_LOG_HEAD_NVRAM_START_ADDRESS (1024*4096)//((64+60)*1024)
...
#define INFUSION_LOG_HEAD_NVRAM_SIZE	(FLASH_ERASE_UNIT_SIZE*4)
//start after head
#define INFUSION_LOG_DATA_NVRAM_START_ADDRESS (INFUSION_LOG_HEAD_NVRAM_START_ADDRESS+INFUSION_LOG_HEAD_NVRAM_SIZE)
//if the nvram's (erase)lifetime is 100,000, 
...
#ifdef INFUSION_LOG_MAX_SIZE
#define INFUSION_LOG_DATA_NVRAM_SIZE	\
	(INFUSION_LOG_MAX_SIZE/(FLASH_ERASE_UNIT_SIZE/(ONE_RECORD_LOG_SIZE+SSZ_NVRAM_CHECKSUM_SIZE))+2)*FLASH_ERASE_UNIT_SIZE
#else
#define INFUSION_LOG_DATA_NVRAM_SIZE	\
	(INFUSION_LOG_MAX_SIZE_AT_LEAST/(FLASH_ERASE_UNIT_SIZE/(ONE_RECORD_LOG_SIZE+SSZ_NVRAM_CHECKSUM_SIZE))+2)*FLASH_ERASE_UNIT_SIZE
#endif


#define ALARM_LOG_HEAD_NVRAM_START_ADDRESS ((1024+200)*4096)//((512+60)*1024)
//if the nvram's (erase)lifetime is 100,000, 
//the 16K(4096*4,can write 4096*4/16=1024 records) size will support over 100 million lifetime
//	detail: (4096*4)/16*100000erase=102,400,000erase
//and it support write one record each 4 second for 10 years.
//	detail: 102400000/10.0/356/24/60=19.97rec/min, 60/19.97=3.004sec/rec
#define ALARM_LOG_HEAD_NVRAM_SIZE	(FLASH_ERASE_UNIT_SIZE*4)
//start after head
#define ALARM_LOG_DATA_NVRAM_START_ADDRESS (ALARM_LOG_HEAD_NVRAM_START_ADDRESS+ALARM_LOG_HEAD_NVRAM_SIZE)
//if the nvram's (erase)lifetime is 100,000, 
//the 1000 records will support over 100 million lifetime
//	detail: 1000*100000erase=100,000,000erase
//and it support write one record each 4 second for 10 years.
...
//because we must erase one unit when write new record, so need add 1 more erase unit(so no record will lost) 
// detail: (1000/(4096/(36+4))+2)*4096/1024=44KB
#ifdef ALARM_LOG_MAX_SIZE
#define ALARM_LOG_DATA_NVRAM_SIZE	\
	(ALARM_LOG_MAX_SIZE/(FLASH_ERASE_UNIT_SIZE/(ONE_RECORD_LOG_SIZE+SSZ_NVRAM_CHECKSUM_SIZE))+2)*FLASH_ERASE_UNIT_SIZE
#else
#define ALARM_LOG_DATA_NVRAM_SIZE	\
	(ALARM_LOG_MAX_SIZE_AT_LEAST/(FLASH_ERASE_UNIT_SIZE/(ONE_RECORD_LOG_SIZE+SSZ_NVRAM_CHECKSUM_SIZE))+2)*FLASH_ERASE_UNIT_SIZE
#endif


#define INTERNAL_LOG1_HEAD_NVRAM_START_ADDRESS ((1024+400)*4096)//((512+60)*1024)
//if the nvram's (erase)lifetime is 100,000, 
//the 16K(4096*4,can write 4096*4/16=1024 records) size will support over 100 million lifetime
//	detail: (4096*4)/16*100000erase=102,400,000erase
//and it support write one record each 4 second for 10 years.
//	detail: 102400000/10.0/356/24/60=19.97rec/min, 60/19.97=3.004sec/rec
#define INTERNAL_LOG1_HEAD_NVRAM_SIZE	(FLASH_ERASE_UNIT_SIZE*4)
//start after head
#define INTERNAL_LOG1_DATA_NVRAM_START_ADDRESS (INTERNAL_LOG1_HEAD_NVRAM_START_ADDRESS+INTERNAL_LOG1_HEAD_NVRAM_SIZE)
//if the nvram's (erase)lifetime is 100,000, 
//the 1000 records will support over 100 million lifetime
//	detail: 1000*100000erase=100,000,000erase
//and it support write one record each 4 second for 10 years.
...
//because we must erase one unit when write new record, so need add 1 more erase unit(so no record will lost) 
// detail: (1000/(4096/(36+4))+2)*4096/1024=44KB
#ifdef INTERNAL_LOG1_MAX_SIZE
#define INTERNAL_LOG1_DATA_NVRAM_SIZE	\
	(INTERNAL_LOG1_MAX_SIZE/(FLASH_ERASE_UNIT_SIZE/(ONE_RECORD_LOG_SIZE+SSZ_NVRAM_CHECKSUM_SIZE))+2)*FLASH_ERASE_UNIT_SIZE
#else
#define INTERNAL_LOG1_DATA_NVRAM_SIZE	\
	(INTERNAL_LOG1_MAX_SIZE_AT_LEAST/(FLASH_ERASE_UNIT_SIZE/(ONE_RECORD_LOG_SIZE+SSZ_NVRAM_CHECKSUM_SIZE))+2)*FLASH_ERASE_UNIT_SIZE
#endif




#define INTERNAL_LOG2_HEAD_NVRAM_START_ADDRESS ((1024+600)*4096)//((512+60)*1024)
//if the nvram's (erase)lifetime is 100,000, 
//the 16K(4096*4,can write 4096*4/16=1024 records) size will support over 100 million lifetime
//	detail: (4096*4)/16*100000erase=102,400,000erase
//and it support write one record each 4 second for 10 years.
//	detail: 102400000/10.0/356/24/60=19.97rec/min, 60/19.97=3.004sec/rec
#define INTERNAL_LOG2_HEAD_NVRAM_SIZE	(FLASH_ERASE_UNIT_SIZE*4)
//start after head
#define INTERNAL_LOG2_DATA_NVRAM_START_ADDRESS (INTERNAL_LOG2_HEAD_NVRAM_START_ADDRESS+INTERNAL_LOG2_HEAD_NVRAM_SIZE)
//if the nvram's (erase)lifetime is 100,000, 
//the 1000 records will support over 100 million lifetime
//	detail: 1000*100000erase=100,000,000erase
//and it support write one record each 4 second for 10 years.
...
//because we must erase one unit when write new record, so need add 1 more erase unit(so no record will lost) 
// detail: (1000/(4096/(36+4))+2)*4096/1024=44KB
#ifdef INTERNAL_LOG2_MAX_SIZE
#define INTERNAL_LOG2_DATA_NVRAM_SIZE	\
	(INTERNAL_LOG2_MAX_SIZE/(FLASH_ERASE_UNIT_SIZE/(ONE_RECORD_LOG_SIZE+SSZ_NVRAM_CHECKSUM_SIZE))+2)*FLASH_ERASE_UNIT_SIZE
#else
#define INTERNAL_LOG2_DATA_NVRAM_SIZE	\
	(INTERNAL_LOG2_MAX_SIZE_AT_LEAST/(FLASH_ERASE_UNIT_SIZE/(ONE_RECORD_LOG_SIZE+SSZ_NVRAM_CHECKSUM_SIZE))+2)*FLASH_ERASE_UNIT_SIZE
#endif
...
SszRecord g_logs[kLogTypeMax];
const static int g_logs_max_size[kLogTypeMax] = {
#ifdef INFUSION_LOG_MAX_SIZE
	INFUSION_LOG_MAX_SIZE,
#else
	-1,
#endif
...
#ifdef INTERNAL_LOG1_MAX_SIZE
	INTERNAL_LOG1_MAX_SIZE,
#else
	-1,
#endif

#ifdef INTERNAL_LOG2_MAX_SIZE
	INTERNAL_LOG2_MAX_SIZE,
#else
	-1,
#endif
};
...
* Function 
************************************************/
int32_t record_log_init() {
	uint32_t ret = 0;
	ssz_traceln("record log init begin");
	
	//infusionLog init
	ssz_record_struct_init(&g_logs[kInfusionLog], data_nvram(), INFUSION_LOG_DATA_NVRAM_START_ADDRESS, 
		INFUSION_LOG_DATA_NVRAM_SIZE, ONE_RECORD_LOG_SIZE, true, true,
		INFUSION_LOG_HEAD_NVRAM_START_ADDRESS, INFUSION_LOG_HEAD_NVRAM_SIZE);
	ssz_assert(g_logs[kInfusionLog].record_user_max_count >= g_logs_max_size[kInfusionLog]);
	if (ssz_record_load(&g_logs[kInfusionLog]) == kSszRecordLastHeadBroken) {
		ssz_trace("infusion log last head broken\n");
		ret = -1;
	}
	...
	//alarm init
	ssz_record_struct_init(&g_logs[kAlarmLog], data_nvram(), ALARM_LOG_DATA_NVRAM_START_ADDRESS,
		ALARM_LOG_DATA_NVRAM_SIZE, ONE_RECORD_LOG_SIZE, true, true,
		ALARM_LOG_HEAD_NVRAM_START_ADDRESS, ALARM_LOG_HEAD_NVRAM_SIZE);
	ssz_assert(g_logs[kAlarmLog].record_user_max_count >= g_logs_max_size[kAlarmLog]);
	if (ssz_record_load(&g_logs[kAlarmLog]) == kSszRecordLastHeadBroken) {
		ssz_trace("alarm log last head broken\n");
		ret = -1;
	}
	...
	//Internal1 init
	ssz_record_struct_init(&g_logs[kInternalLog1], data_nvram(), INTERNAL_LOG1_DATA_NVRAM_START_ADDRESS,
		INTERNAL_LOG1_DATA_NVRAM_SIZE, ONE_RECORD_LOG_SIZE, true, true,
		INTERNAL_LOG1_HEAD_NVRAM_START_ADDRESS, INTERNAL_LOG1_HEAD_NVRAM_SIZE);
	ssz_assert(g_logs[kInternalLog1].record_user_max_count >= g_logs_max_size[kInternalLog1]);
	if (ssz_record_load(&g_logs[kInternalLog1]) == kSszRecordLastHeadBroken) {
		ssz_trace("internal log last head broken\n");
		ret = -1;
	}


	//Internal2 init
	ssz_record_struct_init(&g_logs[kInternalLog2], data_nvram(), INTERNAL_LOG2_DATA_NVRAM_START_ADDRESS,
		INTERNAL_LOG2_DATA_NVRAM_SIZE, ONE_RECORD_LOG_SIZE, true, true,
		INTERNAL_LOG2_HEAD_NVRAM_START_ADDRESS, INTERNAL_LOG2_HEAD_NVRAM_SIZE);
	ssz_assert(g_logs[kInternalLog2].record_user_max_count >= g_logs_max_size[kInternalLog2]);
	if (ssz_record_load(&g_logs[kInternalLog2]) == kSszRecordLastHeadBroken) {
		ssz_trace("internal log last head broken\n");
		ret = -1;
	}
