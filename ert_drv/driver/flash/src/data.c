...
//#define DATA_FACTORY_VERSION 1
//#define DATA_USER_VERSION 1
#define DATA_INFUSION_SET_VERSION	0
#define DATA_ACTION_VERSION			1
#define DATA_ALARM_VERSION			1
#define DATA_FIREWARE_VERSION		1
#define DATA_SYSTEM_SET_VERSION		1
#define DATA_CALIBRATION_VERSION	1
//#define DATA_ENERGY_CALIBRATION_VERSION 1
...
uint8_t g_data_infusion_set_block_cache[156];  //实际152
uint8_t g_data_action_control_block_cache[32]; //实际31
uint8_t g_data_alarm_block_cache[8]; //实际7
uint8_t g_data_fireware_update_block_cache[10]; //实际9
uint8_t g_data_system_set_block_cache[24]; //实际21
uint8_t g_data_calibration_block_cache[138]; //实际136
...
const static DataBlockSaveInfo g_data_block_map[] = {
	...
	//init
	{ kDataInfusionSetInitBlock, kDataTypeInfusionSet, kSszBlockFileNvram, 2, NULL,
		0, FLASH_ERASE_UNIT_SIZE  },
		
	{ kDataActionControlInitBlock, kDataTypeActionControl, kSszBlockFileNvram, 2, NULL,
		DATA_ADDRESS_AUTO, FLASH_ERASE_UNIT_SIZE  },
		
	{ kDataAlarmInitBlock, kDataTypeAlarm, kSszBlockFileNvram, 2, NULL,
		DATA_ADDRESS_AUTO, FLASH_ERASE_UNIT_SIZE  },

	{ kDataFirmwareInitBlock, kDataTypeFirmware, kSszBlockFileNvram, 2, NULL,
		DATA_ADDRESS_AUTO, FLASH_ERASE_UNIT_SIZE  },
		
	{ kDataSystemSetInitBlock, kDataTypeSystemSet, kSszBlockFileNvram, 2, NULL,
		DATA_ADDRESS_AUTO, FLASH_ERASE_UNIT_SIZE  },

	{ kDataCalibrationInitBlock, kDataTypeCalibration, kSszBlockFileNvram, 2, NULL,
		DATA_ADDRESS_AUTO, FLASH_ERASE_UNIT_SIZE  },


	{ kDataInfusionSetBlock, kDataTypeInfusionSet, kSszBlockFileRecord, DATA_DEFINE_SIZE_AND_CACHE(g_data_infusion_set_block_cache),
	  6*FLASH_ERASE_UNIT_SIZE, FLASH_ERASE_UNIT_SIZE*8},


	{ kDataActionControlBlock, kDataTypeActionControl, kSszBlockFileRecord, DATA_DEFINE_SIZE_AND_CACHE(g_data_action_control_block_cache),
	  DATA_ADDRESS_AUTO, FLASH_ERASE_UNIT_SIZE*6},


	{ kDataAlarmBlock, kDataTypeAlarm, kSszBlockFileRecord,  DATA_DEFINE_SIZE_AND_CACHE(g_data_alarm_block_cache),
	  DATA_ADDRESS_AUTO, FLASH_ERASE_UNIT_SIZE*2},


	{ kDataFirmwareBlock, kDataTypeFirmware, kSszBlockFileRecord, DATA_DEFINE_SIZE_AND_CACHE(g_data_fireware_update_block_cache),
	  DATA_ADDRESS_AUTO, FLASH_ERASE_UNIT_SIZE*2},


	{ kDataSystemSetBlock, kDataTypeSystemSet, kSszBlockFileRecord, DATA_DEFINE_SIZE_AND_CACHE(g_data_system_set_block_cache),
	  DATA_ADDRESS_AUTO, FLASH_ERASE_UNIT_SIZE*4},
	  

	{ kDataCalibrationBlock, kDataTypeCalibration, kSszBlockFileRecord, DATA_DEFINE_SIZE_AND_CACHE(g_data_calibration_block_cache),
	  DATA_ADDRESS_AUTO, FLASH_ERASE_UNIT_SIZE*8},
...
const static DataChildSaveInfo g_data_child_map[] = {
	...
	{ kDataDoseCtrolParam, 7, DATA_ADDRESS_AUTO },
	{ kDataBaseSet, 48, DATA_ADDRESS_AUTO },
	{ kDataTempSet, 48, DATA_ADDRESS_AUTO },
	{ kDataLastTempSet, 1, DATA_ADDRESS_AUTO },
	{ kDataBolusSet, 48, DATA_ADDRESS_AUTO },

	{ kDataFirstLoadDose, 1, DATA_ADDRESS_AUTO },
	{ kDataInfusionSwtich, 1, DATA_ADDRESS_AUTO },
	{ kDataBaseProData, 6, DATA_ADDRESS_AUTO },
	{ kDataBolusProData, 6, DATA_ADDRESS_AUTO },
	{ kDataDelayProData, 17, DATA_ADDRESS_AUTO },
	
	{ kDataAlarmFlag, 1, DATA_ADDRESS_AUTO },
	{ kDataAlarmConfig, 6, DATA_ADDRESS_AUTO },

	{ kDataFirmwareInfo, 9, DATA_ADDRESS_AUTO },

	{ kDataInfusionForce, 1, DATA_ADDRESS_AUTO },
	{ kDataSerialNumber, 10, DATA_ADDRESS_AUTO },
	{ kDataSoftwareVersion, 6, DATA_ADDRESS_AUTO },
	{ kDataHardwareVersion, 3, DATA_ADDRESS_AUTO },
	{ kDataFlashConfigFlag, 1, DATA_ADDRESS_AUTO },

    { kDatabetteryOffset, 4, DATA_ADDRESS_AUTO },
	{ kDataCalibrationInfo, 132, DATA_ADDRESS_AUTO },
	
};
	
static SszBlockFile g_data_block_files[sizeof(g_data_block_map) / sizeof(g_data_block_map[0])];

SszNvram g_data_file_nvram;
...
void data_on_read_error(DataID read_data_id, int error_code, uint8_t notify_error_flags) {
	...
	if (is_need_notify)
	{
		...
		if (g_data_block_map[block_index].data_id == kDataUserBlock) {
...
void data_first_init_if_need()
{
	...
	bool is_need_init;
	const DataID init_ids[] = {kDataInfusionSetInitBlock, kDataActionControlInitBlock, kDataAlarmInitBlock, kDataFirmwareInitBlock, kDataSystemSetInitBlock, kDataCalibrationInitBlock};//{kDataFactoryInitBlock, kDataUserInitBlock };
	const int init_id_versions[] = {DATA_INFUSION_SET_VERSION, DATA_ACTION_VERSION, DATA_ALARM_VERSION, DATA_FIREWARE_VERSION, DATA_SYSTEM_SET_VERSION, DATA_CALIBRATION_VERSION};//{ DATA_FCT_VERSION, DATA_FACTORY_VERSION, DATA_USER_VERSION};

	const DataType init_data_type[] = {kDataTypeInfusionSet, kDataTypeActionControl, kDataTypeAlarm, kDataTypeFirmware, kDataTypeSystemSet, kDataTypeCalibration};
...
int32_t data_init() {
	...
	ssz_nvram_init(&g_data_file_nvram, drv_sst26_flash_file(), true, DATA_TRY_COUNT,
