...
static void _cbDialog(WM_MESSAGE * pMsg)
{
	...
	switch (pMsg->MsgId) 
	{
		case WM_INIT_DIALOG:
			...
			hWin = widget_text_create(DISPLAY_WIDTH/2+16+len, 30, DISPLAY_WIDTH/2 - len, 16, pMsg->hWin, WM_CF_SHOW, "20/230ml");
			...
			len = GUI_GetStringDistX(p);
			hWin = widget_text_create(DISPLAY_WIDTH/2+16+len, 48, DISPLAY_WIDTH/2 - len, 16, pMsg->hWin, WM_CF_SHOW, "50ml/hr");
			...
			//left time
			hWin = widget_text_create(0, 0, DISPLAY_WIDTH/2 - 16, 64, pMsg->hWin, WM_CF_SHOW, "46");
			ui_set_child_hwin(INFUSION_ID_STR_TIME, hWin);
			widget_text_set_font(hWin, get_font(88));
			widget_text_set_color(hWin, GUI_WHITE,GUI_BLACK);
			widget_text_set_align(hWin, GUI_TA_VCENTER|GUI_TA_HCENTER);
		
			break;
	
		case WM_PAINT:
			GUI_SetFont(get_font(14));
			ui_rect_init_by_size(rect, DISPLAY_WIDTH/2 - 16, DISPLAY_HEIGHT-18, 16, 18);
			...
			p = get_string(kStrInfusionSpeed);
			len = GUI_GetStringDistX(p);			
			ui_rect_init_by_size(rect, DISPLAY_WIDTH/2+16, 48, len, 16);
			GUI_DispStringInRect(p, &rect, GUI_TA_VCENTER|GUI_TA_HCENTER);
