...
#define ID_CURRENT_MIN_1			(GUI_ID_USER + 0x01)
#define ID_CURRENT_MIN_2			(GUI_ID_USER + 0x02)
#define ID_CURRENT_MIN_3			(GUI_ID_USER + 0x03)
#define ID_CURRENT_MIN_4			(GUI_ID_USER + 0x04)
#define ID_CURRENT_MIN_5			(GUI_ID_USER + 0x05)

#define ID_CURRENT_MIN_IMG_SELECT	(GUI_ID_USER + 0x06)
...
static void move_current_min_cusor(int cursor)
{
	WM_HWIN hWin;
	if((cursor < 1) || (cursor > 5)) return;
	hWin = WM_GetDialogItem(ui_get_current_hwin(), ID_CURRENT_MIN_IMG_SELECT);
	WM_MoveChildTo(hWin, get_image_x(cursor), 52);
}

static void change_current_min_data(int cursor, int data)
{
	WM_HWIN hWin;
	char buff[4];
	if((cursor < 1) || (cursor > 5)) return;
	...
	switch (pMsg->MsgId) 
	{
		case WM_INIT_DIALOG:
			...
			hWin = IMAGE_CreateEx(get_image_x(0), 52, bitmap->XSize, bitmap->YSize, pMsg->hWin, 
				                  WM_CF_SHOW,  IMAGE_CF_AUTOSIZE, ID_CURRENT_MIN_IMG_SELECT);
...
int app_scr_test_current_min_update(WM_HWIN hwin, type_MsgBody4UICtrlMsg *msg)
{
	switch(msg->OpMethod)
	{
		case SCR_CURSER_MOVE:
			move_current_min_cusor(msg->CursorIndex);
			break;

		case SCR_VALUE_ALTER:
			move_current_min_cusor(msg->CursorIndex);
			change_current_min_data(msg->CursorIndex, msg->DItem[0]);
			break;
		default:
			break;
	}
