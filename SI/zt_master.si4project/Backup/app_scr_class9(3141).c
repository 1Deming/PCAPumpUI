/*********************************************************************
*                                                                    *
*                SEGGER Microcontroller GmbH & Co. KG                *
*        Solutions for real time microcontroller applications        *
*                                                                    *
**********************************************************************
*                                                                    *
* C-file generated by:                                               *
*                                                                    *
*        GUI_Builder for emWin version 5.44                          *
*        Compiled Nov 10 2017, 08:53:57                              *
*        (c) 2017 Segger Microcontroller GmbH & Co. KG               *
*                                                                    *
**********************************************************************
*                                                                    *
*        Internet: www.segger.com  Support: support@segger.com       *
*                                                                    *
***********************************************************************/

#include "DIALOG.h"
#include "GUI.h"
#include "ui_common.h"
#include "image_data.h"
#include "string_data.h"
#include "font_data.h"
#include <stdbool.h>
#include "app_scr_class9.h"
#include <stdio.h>
#include "ssz_common.h"
#include "widget_text.h"
#include "widget_image.h"


/*********************************************************************
*
*       Defines
*
***********************************************************************/
#define ID_WINDOW_0     (GUI_ID_USER + 0x00)

#define MessageID1	(GUI_ID_USER + 0x01)
#define MessageID2	(GUI_ID_USER + 0x02)
#define MessageID3	(GUI_ID_USER + 0x03)


/*********************************************************************
*
*       Static data
*
***********************************************************************/
static const GUI_WIDGET_CREATE_INFO _aDialogCreate[] = {
  {WINDOW_CreateIndirect, "app_scr_class9", ID_WINDOW_0, 0, 0, 256, 64, 0, 0x0, 0},
  
};
static int g_home_left_time;
/*********************************************************************
*
*       Static code
*
***********************************************************************/

static void put_message(uint8_t id , uint8_t num ,uint8_t num2 , uint8_t unit , uint8_t cursor ,double KVO_rate)
{		
	char p[30];
    const char *ptr;
    WM_HWIN hWin;
    
    hWin = WM_GetDialogItem(ui_get_current_hwin(), MessageID1);
	{
		switch(id)
			{
				case 0:
					TEXT_SetText(hWin, get_string(kStrrate));		
					break;
				case 1:
					TEXT_SetText(hWin, get_string(kStrvolume));	
					break;
				default:
					
					break;			
			}
    }
			
    hWin = WM_GetDialogItem(ui_get_current_hwin(), MessageID2);
	sprintf(p, "%d/%d", num,num2);	
	TEXT_SetText(hWin, p);	


    hWin = WM_GetDialogItem(ui_get_current_hwin(), MessageID3);
    sprintf(p, "%4.1f", KVO_rate);	
    if(unit == 1)
    {
      strcat(p," mL/hr");
    }
    else
    {
      strcat(p," mg/hr");
    }
        
    if(cursor == 1)
    {
        TEXT_SetBkColor(hWin,GUI_WHITE);
        TEXT_SetTextColor(hWin,GUI_BLACK);
    }
    else
    {
        TEXT_SetBkColor(hWin,GUI_BLACK);
        TEXT_SetTextColor(hWin,GUI_WHITE);
    }

    TEXT_SetText(hWin, p);
  
//    GUI_RECT rect;
//    sprintf(p, "%4.1f", KVO_rate);	
//    if(unit == 1)
//    {
//      strcat(p," mL/hr");
//    }
//    else
//    {
//      strcat(p," mg/hr");
//    }
//    ui_rect_init_by_size(rect, 89, 25, GUI_GetStringDistX("1251.2 mL/hr"), 14);
//    GUI_SetTextStyle(GUI_TS_UNDERLINE);
//    GUI_DispStringInRect(p, &rect, GUI_TA_LEFT|GUI_TA_VCENTER);

}

static void _cbDialog(WM_MESSAGE * pMsg)
{
	GUI_RECT rect;
	WM_HWIN hWin;
	int len;
	const char *ptr;
        char *p;
        uint8_t strlen;
        
	switch (pMsg->MsgId) 
	{
		case WM_CREATE:
			
			break;
			
		case WM_INIT_DIALOG:
			WINDOW_SetBkColor(pMsg->hWin, GUI_BLACK);

			GUI_SetFont(get_font(14));
			ptr = get_string(kStrrate);
			strlen = GUI_GetStringDistX(ptr);              
			hWin = TEXT_CreateEx(6, 6, strlen, 14, pMsg->hWin, 
				                 WM_CF_SHOW, GUI_TA_VCENTER|GUI_TA_HCENTER, MessageID1, 0);
			TEXT_SetFont(hWin, get_font(14));
			TEXT_SetBkColor(hWin,GUI_BLACK );
			TEXT_SetTextColor(hWin, GUI_WHITE);
			WM_BringToTop(hWin);

			
			strlen = GUI_GetStringDistX("1/3");                                               
			hWin = TEXT_CreateEx(220, 6, strlen, 14, pMsg->hWin, 
				                 WM_CF_SHOW, GUI_TA_VCENTER|GUI_TA_HCENTER, MessageID2, 0);
			TEXT_SetFont(hWin, get_font(14));
			TEXT_SetBkColor(hWin,GUI_BLACK );
			TEXT_SetTextColor(hWin, GUI_WHITE);
			WM_BringToTop(hWin);


			strlen = GUI_GetStringDistX("1251.2 mL/hr");                                               
			hWin = TEXT_CreateEx(89, 25, strlen, 14, pMsg->hWin, 
				                 WM_CF_SHOW, GUI_TA_VCENTER|GUI_TA_HCENTER, MessageID3, 0);
			TEXT_SetFont(hWin, get_font(14));
			TEXT_SetBkColor(hWin,GUI_BLACK );
			TEXT_SetTextColor(hWin, GUI_WHITE);
			WM_BringToTop(hWin);

			break;
	
		case WM_PAINT:
			GUI_SetColor(GUI_WHITE);
			GUI_SetFont(get_font(14));
				
			ptr = get_string(kStrReturn);
			strlen = GUI_GetStringDistX(ptr);
			ui_rect_init_by_size(rect, 2, 46, strlen, 14);
			GUI_DispStringInRect(ptr, &rect, GUI_TA_LEFT|GUI_TA_VCENTER);

			ptr = get_string(kStredit);
			strlen = GUI_GetStringDistX(ptr);
			ui_rect_init_by_size(rect, 220, 46, strlen, 14);
			GUI_DispStringInRect(ptr, &rect, GUI_TA_LEFT|GUI_TA_VCENTER);
			 
                        
                       
			break; 
		
		default:
			  WM_DefaultProc(pMsg);
			  break;
	}
}

/*********************************************************************
*
*       Public code
*
***********************************************************************/
WM_HWIN app_scr_class9_create(type_MsgBody4UICtrlMsg *msg) 
{
	WM_HWIN hWin;

	hWin = GUI_CreateDialogBox(_aDialogCreate, GUI_COUNTOF(_aDialogCreate), _cbDialog, WM_HBKWIN, 0, 0);
	
	return hWin;
}


int app_scr_class9_update(WM_HWIN hwin, type_MsgBody4UICtrlMsg *msg)
{
	//g_home_left_time = msg->SItem.DataValArray[4];
	//put_infusion_state(msg->SItem.DataValArray[5]);
	//put_infusion_dose(msg->SItem.DataValArray[6], msg->SItem.DataValArray[7]);
	//put_infusion_seed(msg->SItem.DataValArray[8]);
	put_message(msg->SItem.DataValArray[1],msg->SItem.DataValArray[3],msg->SItem.DataValArray[4],msg->SItem.DataValArray[2],msg->CursorIndex.CursorType,1549.5);
 	return 0;	
}

void app_scr_class9_destroy(WM_HWIN win_id) 
{
	GUI_EndDialog(win_id, 0);
}

/*************************** End of file ****************************/

