...
*/
static uint8_t data;
static uint8_t index = SCR_CURSER_MOVE;
void test(void)
{
	type_MsgBody4UICtrlMsg disp_msg;
	const ScreenInfo *sptr;

	sptr = (const ScreenInfo *)&g_scr_routes[3];


	data = (data +1)%3;
	memcpy(disp_msg.UIID, sptr->screen_id, 5);
	disp_msg.CursorIndex = data;
	disp_msg.OpMethod = SCR_CURSER_MOVE;
	disp_msg.DItem[0] = 0;
	ert_uiDisp(SCR_CURSER_MOVE, &disp_msg);
...
/*
   ...
   GUI_DrawBitmap(get_image(kImgBatteryThree), 180, 8);

   
 	GUI_DispStringAt("Spinrg", 0, 30);
    GUI_SetFont(get_font(14));
	GUI_DispStringAt(get_string(kStrFactoryMode), 180, 45);
*/
	memcpy(disp_msg.UIID, sptr->screen_id, 5);
	disp_msg.CursorIndex = 0;
	disp_msg.OpMethod = SCR_CREATE;
	disp_msg.DItem[0] = 0;
	ert_uiDisp(SCR_CREATE, &disp_msg);


	timer_set_handler(kTimerTest, test);
	timer_start_periodic_every(kTimerTest, 500);
