...
static uint8_t index = SCR_CREATE;
void test(void)
{
	...
	switch(index)
	{
		case SCR_CREATE:
			...
			index = SCR_CURSER_MOVE;
			break;

		case SCR_CURSER_MOVE:
			memcpy(disp_msg.UIID, sptr->screen_id, 5);
			disp_msg.CursorIndex = 2;
			disp_msg.OpMethod = SCR_CURSER_MOVE;
			disp_msg.DItem[0] = 0;
			index = SCR_VALUE_ALTER;
			break;
...
*/
	...
	timer_start_periodic_every(kTimerTest, 5000);
