...
static void put_left_time(uint16_t time)
{
	...
	ui_rect_init_by_size(rect, x, 0, dlen+hlen, 64);
	WM_InvalidateRect(g_ui_common_param.win_id, &rect);
}
...
static void put_infusion_dose(uint8_t total_dose, uint8_t infusioned_dose)
{
	...
	strcat(p, "ml");
	widget_text_set_str(ui_get_child_hwin(INFUSION_ID_STR_DOSE), p);
}
...
static void put_infusion_seed(uint8_t infusion_speed)
{
	...
	strcat(p, "ml/hr");
	widget_text_set_str(ui_get_child_hwin(INFUSION_ID_STR_SPEED), p);
}
...
static void _cbDialog(WM_MESSAGE * pMsg)
{
	...
	switch (pMsg->MsgId) 
	{
		case WM_INIT_DIALOG:
			...
			//state string
			hWin = widget_text_create(DISPLAY_WIDTH/2+24, 12, DISPLAY_WIDTH/2 - 24, 16, pMsg->hWin, WM_CF_SHOW, get_string(kStrInfusing));
			...
			//state image
			hWin = widget_image_create(DISPLAY_WIDTH/2+24, 12, 16, 16, pMsg->hWin, WM_CF_SHOW, get_image(kImgInfusionRun));
			...
			hWin = widget_text_create(DISPLAY_WIDTH/2+24+len, 30, DISPLAY_WIDTH/2 - len-24, 16, pMsg->hWin, WM_CF_SHOW, "999/230ml");
			...
			len = GUI_GetStringDistX(p);
			hWin = widget_text_create(DISPLAY_WIDTH/2+24+len, 48, DISPLAY_WIDTH/2 - len-24, 16, pMsg->hWin, WM_CF_SHOW, "50ml/hr");
