...
//      //
  //PixelIndex = 0xFF;
  	  ...
  	  //ui_set_fill_color(PixelIndex);
      //display_data_at(x, y, (uint8_t *)&PixelIndex , 1);
	  //ui_draw_pixel_at(x, y);
	  ui_draw_pixel_with_color_at(x, y, PixelIndex);
//    }
...
static unsigned long _GetPixelIndex(GUI_DEVICE * pDevice, int x, int y) {
  unsigned int PixelIndex;
    ...
    GUI_USE_PARA(y);
    {
      ...
      PixelIndex =  display_data_get(x, y);
    }
    #if (LCD_MIRROR_X == 0) && (LCD_MIRROR_Y == 0) && (LCD_SWAP_XY == 0)
      #undef xPhys
      #undef yPhys
...
////
  ...
  //ui_set_fill_color(PixelIndex);
//
 //ui_fill_rect_at(x0, y0, x1-x0+1, y1-y0+1);
}
...
static void * _GetDevData(GUI_DEVICE * pDevice, int Index) {
  GUI_USE_PARA(pDevice);
    ...
    case LCD_DEVDATA_MEMDEV:
      return (void *)&GUI_MEMDEV_DEVICE_8; // TBD: Has to be adapted to the right memory device depending on the used color depth!
    }
  #else
    GUI_USE_PARA(Index);
  #endif
