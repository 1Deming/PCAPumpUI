...
static void put_lock_state(uint8_t state)
{
	...
	IMAGE_SetBitmap(WM_GetDialogItem(g_ui_common_param.win_id, LOCK_STATE), bitmap);
}
	...
	switch (pMsg->MsgId) 
	{
		...
		case WM_INIT_DIALOG:
			...
			hWin = TEXT_CreateEx(0, 0, 130, 64, pMsg->hWin, 
				                 WM_CF_SHOW, TEXT_CF_HCENTER,  LEFT_TIME_ID, "255");
			...
			//lock state
			hWin = IMAGE_CreateEx(DISPLAY_WIDTH/2+24, 14, 16, 12, pMsg->hWin, WM_CF_SHOW,  IMAGE_CF_AUTOSIZE, LOCK_STATE);
