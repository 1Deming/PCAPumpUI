...
static void _cbDialog(WM_MESSAGE * pMsg)
{
	...
	switch (pMsg->MsgId) 
	{
		case WM_INIT_DIALOG:
			...
			//state image
			hWin = IMAGE_CreateEx(0, (DISPLAY_HEIGHT-IMAGE_BAG_HEIGHT)/2, 256, IMAGE_BAG_HEIGHT, pMsg->hWin, WM_CF_SHOW,  IMAGE_CF_AUTOSIZE, EXHAUST_ID_IMG_SELECT);
			IMAGE_SetBitmap(hWin, get_image(kImgSelect1));


			//state string
			GUI_SetFont(get_font(14));
			len = GUI_GetStringDistX(get_string(kStrMedsBag1));
			hWin = TEXT_CreateEx((DISPLAY_WIDTH/2 - len)/2, (DISPLAY_HEIGHT-IMAGE_BAG_HEIGHT)/2, len, IMAGE_BAG_HEIGHT, pMsg->hWin, 
