/*********************************************************************
*                                                                    *
*                SEGGER Microcontroller GmbH & Co. KG                *
*        Solutions for real time microcontroller applications        *
*                                                                    *
**********************************************************************
*                                                                    *
* C-file generated by:                                               *
*                                                                    *
*        GUI_Builder for emWin version 5.44                          *
*        Compiled Nov 10 2017, 08:53:57                              *
*        (c) 2017 Segger Microcontroller GmbH & Co. KG               *
*                                                                    *
**********************************************************************
*                                                                    *
*        Internet: www.segger.com  Support: support@segger.com       *
*                                                                    *
**********************************************************************
*/


#include "DIALOG.h"
#include "GUI.h"
#include "ui_common.h"
#include "image_data.h"
#include "string_data.h"
#include "font_data.h"
#include <stdbool.h>
#include "app_scr_stop_infusion_13100.h"
#include <stdio.h>


/*********************************************************************
*
*       Defines
*
**********************************************************************
*/
#define ID_WINDOW_0         		 (GUI_ID_USER + 0x00)
#define STOP_INFUSION_ID_STR_LEFT     (GUI_ID_USER + 0x01)
#define STOP_INFUSION_ID_STR_RIGHT      (GUI_ID_USER + 0x02)
#define STOP_INFUSION_ID_IMG         (GUI_ID_USER + 0x03)


#define STOP_INFUSION_ID_W			160
#define STOP_INFUSION_ID_H          24


/*********************************************************************
*
*       Static data
*
**********************************************************************
*/


static const GUI_WIDGET_CREATE_INFO _aDialogCreate[] = {
  { WINDOW_CreateIndirect, "app_scr_stop_infusion_13100", ID_WINDOW_0, 0, 0, 256, 64, 0, 0x0, 0 },
};

/*********************************************************************
*
*       Static code
*
**********************************************************************
*/
static void _cbDialog(WM_MESSAGE * pMsg)
{
	uint8_t strlen;
	GUI_RECT rect; 
	const char *ptr;
	WM_HWIN hWin;
	int x,y;

	
	switch (pMsg->MsgId) 
	{
		case WM_INIT_DIALOG:
			WINDOW_SetBkColor(pMsg->hWin, GUI_BLACK);

		    x = (DISPLAY_WIDTH - STOP_INFUSION_ID_W)/2;
			y = 20;

			//alarm image
			hWin = IMAGE_CreateEx(x, y, STOP_INFUSION_ID_W, STOP_INFUSION_ID_H, pMsg->hWin, WM_CF_SHOW,  
			                      IMAGE_CF_AUTOSIZE, STOP_INFUSION_ID_IMG);
			IMAGE_SetBitmap(hWin, get_image(kImgSelect1));


			ptr = get_string(kStrYes);
			strlen = GUI_GetStringDistX(ptr);
			//yes
			hWin = TEXT_CreateEx(x + (STOP_INFUSION_ID_W/2 - strlen)/2, y+5, strlen, 14, pMsg->hWin, 
				                 WM_CF_SHOW, GUI_TA_VCENTER|GUI_TA_HCENTER,  STOP_INFUSION_ID_STR_LEFT, get_string(kStrYes));
			TEXT_SetFont(hWin, get_font(14));
			TEXT_SetBkColor(hWin, GUI_WHITE);
			TEXT_SetTextColor(hWin, GUI_BLACK);
			WM_BringToTop(hWin);
			
			//No
			hWin = TEXT_CreateEx(x + STOP_INFUSION_ID_W/2+(STOP_INFUSION_ID_W/2 - strlen)/2, y+5, strlen, 14, pMsg->hWin,  
				                 WM_CF_SHOW, GUI_TA_VCENTER|GUI_TA_HCENTER,  STOP_INFUSION_ID_STR_RIGHT, get_string(kStrNo));
			TEXT_SetFont(hWin, get_font(14));
			TEXT_SetBkColor(hWin, GUI_WHITE);
			TEXT_SetTextColor(hWin, GUI_BLACK);
			WM_BringToTop(hWin);
			break;
	
		case WM_PAINT:
			GUI_SetColor(GUI_WHITE);

			GUI_SetFont(get_font(14));
				
			ptr = get_string(kStrWhetherToStopInfuse);
			strlen = GUI_GetStringDistX(ptr);
			ui_rect_init_by_size(rect, 0, 0, strlen, 12);
			GUI_DispStringInRect(ptr, &rect, GUI_TA_LEFT|GUI_TA_VCENTER);

			ptr = get_string(kStrReturn);
			strlen = GUI_GetStringDistX(ptr);
			ui_rect_init_by_size(rect, 0, DISPLAY_HEIGHT-16, strlen, 16);
			GUI_DispStringInRect(ptr, &rect, GUI_TA_LEFT|GUI_TA_VCENTER);

			ptr = get_string(kStrConfirm);
			strlen = GUI_GetStringDistX(ptr);
			ui_rect_init_by_size(rect, DISPLAY_WIDTH-strlen, DISPLAY_HEIGHT-16, strlen, 16);
			GUI_DispStringInRect(ptr, &rect, GUI_TA_RIGHT|GUI_TA_VCENTER);
			
			break; 
			
		default:
			  WM_DefaultProc(pMsg);
			  break;
	}




}

/*********************************************************************
*
*       Public code
*
***********************************************************************/
WM_HWIN app_scr_stop_infusion_13100_create(type_MsgBody4UICtrlMsg *msg) 
{
	WM_HWIN hWin;	
	hWin = GUI_CreateDialogBox(_aDialogCreate, GUI_COUNTOF(_aDialogCreate), _cbDialog, WM_HBKWIN, 0, 0);
	return hWin;
}


int app_scr_stop_infusion_13100_update(WM_HWIN hwin, type_MsgBody4UICtrlMsg *msg)
{
	switch(msg->OpMethod)
	{
		case SCR_CURSER_MOVE:
			
			break;

		case SCR_VALUE_ALTER:

			break;

		default:
			break;
	}

 	return 0;	
}

void app_scr_stop_infusion_13100_destroy(WM_HWIN win_id) 
{
	GUI_EndDialog(win_id, 0);
}


/*************************** End of file ****************************/
