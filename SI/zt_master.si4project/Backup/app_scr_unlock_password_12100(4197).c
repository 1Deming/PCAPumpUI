...
void put_select_arrow(uint8_t cursor_index)
{
	...
	WM_MoveChildTo(WM_GetDialogItem(ui_get_current_hwin(), UNLOCK_ID_IMG), rect.x0, rect.y0);
}
...
static void _cbDialog(WM_MESSAGE * pMsg) 
{
	...
	switch (pMsg->MsgId) 
	{
		case WM_INIT_DIALOG:
			...
			//str1
			get_char_coordinate(0, &rect);
			hWin = TEXT_CreateAsChild(rect.x0, rect.y0, rect.x1 - rect.x0 + 1, rect.y1 - rect.y0 + 1, pMsg->hWin, UNLOCK_ID_STR_1, 
			...
			//str1		
			get_char_coordinate(1, &rect);
			hWin = TEXT_CreateAsChild(rect.x0, rect.y0, rect.x1 - rect.x0 + 1, rect.y1 - rect.y0 + 1, pMsg->hWin, UNLOCK_ID_STR_2, 
				                      WM_CF_SHOW, "0", GUI_TA_VCENTER|GUI_TA_HCENTER);
			TEXT_SetFont(hWin, get_font(60));
			TEXT_SetBkColor(hWin, GUI_BLACK);
			TEXT_SetTextColor(hWin, GUI_WHITE);
			...
			//str1
			get_char_coordinate(2, &rect);
			hWin = TEXT_CreateAsChild(rect.x0, rect.y0, rect.x1 - rect.x0 + 1, rect.y1 - rect.y0 + 1, pMsg->hWin, UNLOCK_ID_STR_3, 
				                      WM_CF_SHOW, "0", GUI_TA_VCENTER|GUI_TA_HCENTER);
			TEXT_SetFont(hWin, get_font(60));
			TEXT_SetBkColor(hWin, GUI_BLACK);
			TEXT_SetTextColor(hWin, GUI_WHITE);
			...
			hWin = IMAGE_CreateEx(rect.x0, rect.y0, rect.x1 - rect.x0 + 1, rect.y1 - rect.y0 + 1, pMsg->hWin, 
				                  WM_CF_SHOW,  IMAGE_CF_AUTOSIZE, UNLOCK_ID_IMG);
			IMAGE_SetBitmap(hWin, get_image(kImgRightArrow));
...
int app_scr_unlock_password_12100_update(WM_HWIN hwin, type_MsgBody4UICtrlMsg *msg)
{
	...
	switch(msg->OpMethod)
	{
		case SCR_CURSER_MOVE:
			put_select_arrow(msg->CursorIndex);
			break;
